 ...
 */
"use strict";
app.controller('venueTabs', ['$log', '$scope', '$http', '$location', 'RestURL', 'DataShare', '$window', '$routeParams', 'AjaxService', 'APP_ARRAYS', 'APP_COLORS', '$rootScope',
    function ($log, $scope, $http, $location, RestURL, DataShare, $window, $routeParams, AjaxService, APP_ARRAYS, APP_COLORS, $rootScope) {
        ...
        self.dispatchToService = function(serviceName) {
            if(self.imageParam === 'Y' || self.imageParam === 'y') {
                $location.url("/newCities/"+ $routeParams.cityName + "/" + $routeParams.venueid + "/" + serviceName +"/new" + "?i=Y");
            } else if(serviceName === undefined) {
                $location.url("/newCities/"+ $routeParams.cityName + "/" + $routeParams.venueid + "/bottle-service" + "/new");
            } else {
